...
export default async function handler(request, response) {
  ...
  try {
    ...
    // --- ИСПРАВЛЕНИЕ: ИСПОЛЬЗУЕМ ПРАВИЛЬНЫЙ URL ИЗ ЛОГА ---
    const hfResponse = await fetch(
      "https://router.huggingface.co/hf-inference", // <-- Вот правильный URL
      {
        headers: {
          "Authorization": `Bearer ${HF_TOKEN}`,
          "Content-Type": "application/json"
        },
        method: "POST", // <-- Синтаксическая ошибка исправлена
        body: JSON.stringify({
          model: "google/gemma-7b-it", // Модель, которую мы используем
          inputs: inputs,
